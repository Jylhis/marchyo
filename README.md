...
The dark variant uses the Jylhis Field palette; the light variant uses Jylhis Sheet (both derived from the [Jylhis Design System](https://github.com/jylhis/design) `tokens.json`). Set `marchyo.theme.scheme = "<name>"` to use a `base16-schemes` scheme instead.
