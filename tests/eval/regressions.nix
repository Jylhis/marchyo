...
in
{
  ...
  # Desktop surfaces (Hyprland) must NOT be installed when the desktop stack is
  # disabled, and MUST be when it is enabled. Matches the documented
  # marchyo.desktop.enable contract.
  eval-desktop-off-no-hyprland =
    testNixOSCheck "desktop-off-no-hyprland" (c: c.programs.hyprland.enable == false)
      (withTestUser {
        marchyo.desktop.enable = false;
      });

  eval-desktop-on-has-hyprland =
    testNixOSCheck "desktop-on-has-hyprland" (c: c.programs.hyprland.enable == true)
      (withTestUser {
        marchyo.desktop.enable = true;
      });

  # The 1Password CLI stays available headless; only the GUI is desktop-gated.
  eval-desktop-off-keeps-1password-cli =
    testNixOSCheck "desktop-off-1password-cli"
      (c: c.programs._1password.enable == true && c.programs._1password-gui.enable == false)
      (withTestUser {
        marchyo.desktop.enable = false;
      });

  # media.enable now actually gates the media apps (previously a dead flag).
  eval-media-disabled = testNixOS "media-disabled" (withTestUser {
    marchyo.desktop.enable = false;
