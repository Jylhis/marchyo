...
```
tests/
  default.nix        # Entry point; auto-discovers tests/eval/*.nix and appends lib-tests.nix
  lib.nix            # Shared helpers (testNixOS, testNixOSCheck, withTestUser)
  eval/*.nix         # Per-feature module evaluation tests (auto-discovered)
...
Drop a new file (or extend an existing one) under `tests/eval/`. Each file receives the shared `helpers` and returns an attrset of named tests:
...
Tests are auto-discovered from `tests/eval/`, so no entry-point edit is needed. The `testNixOS` helper evaluates the NixOS config without building derivations. The `withTestUser` helper merges your config with a minimal bootable config (grub disabled, root filesystem, stateVersion).
