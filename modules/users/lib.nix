...
let
  ...
  # All members of a group as it renders on `platform`, as login names:
  # explicit `groups.<name>.members` merged with every user listing the
  # group in its per-user `groups`. Only users actually rendered on the
  # platform (enabled + platform-matched, mirroring filterUsers) are
  # listed — a group must never name an account that does not exist on the
  # host. Member entries may be identity attribute names (translated to the
  # identity's `username`) or literal login names of users not managed by
  # this module (kept as-is).
  groupMembersOf =
    cfg: platform: groupName:
    let
      rendered = filterUsers { inherit cfg platform; };
      toUsername =
        m:
        if cfg.users ? ${m} then
          # Declared identity: listed only when rendered on this platform.
          lib.optional (rendered ? ${m}) rendered.${m}.username
        else
          [ m ];
      fromGroup = lib.concatMap toUsername (cfg.groups.${groupName} or { members = [ ]; }).members;
      fromUsers = lib.mapAttrsToList (_name: u: u.username) (
        lib.filterAttrs (_name: u: lib.elem groupName u.groups) rendered
      );
    in
