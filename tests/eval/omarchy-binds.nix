  ...
  eval-omarchy-binds-enabled =
    ...
    pkgs.writeText "eval-omarchy-binds-enabled" (
      if
        ...
        && hasPackage hm "marchyo-monitor-scale-cycle"
        && hasPackage hm "marchyo-laptop-display-toggle"
        && hasPackage hm "marchyo-file-manager-cwd"
        && hasPackage hm "tmux"
      then
        "pass"
      else
  ...
  eval-omarchy-binds-disabled =
    ...
    pkgs.writeText "eval-omarchy-binds-disabled" (
      if
        ...
        && !(hasBind hm "marchyo-laptop-display-toggle")
        && !(hasBind hm "marchyo-file-manager-cwd")
        && !(hasPackage hm "tmux")
      then
        "pass"
      else
