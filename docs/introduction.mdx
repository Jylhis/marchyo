...
Every custom option lives under the `marchyo.*` namespace, declared across the auto-discovered files in `modules/nixos/options/` (one file per logical namespace). Browse the [Configuration](/configuration/feature-flags) section for the full reference.
