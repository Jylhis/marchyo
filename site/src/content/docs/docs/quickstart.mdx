...
If you only want the Home Manager modules without the NixOS system modules, use
...
vicinae, sops-nix), and a mock `osConfig` — a plain
...
The builder takes only `system` and `homeDirectory`, and currently manages a
