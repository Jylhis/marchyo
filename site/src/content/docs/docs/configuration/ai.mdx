...
| `marchyo.ai.routing.tools` | `{ pi = "everydayCoding"; }` | Which task bucket each OpenRouter-backed client defaults to |
...
marchyo.ai.routing.tools.pi = "frontier";   # which bucket pi uses
```

Buckets: `frontier`, `everydayCoding`, `fast`, `reasoning`, `summarize`,
`longContext`, `budget`, `local` (placeholder). pi defaults to the
`everydayCoding` bucket (`marchyo.ai.routing.tools`). The resolved policy is
exported to `~/.config/marchyo/ai-routing.json`.
...
standard natively. Add more skills by dropping
