...
in
{
  ...
  # compositor, cage hosts quickshell's layer-shell surfaces, and quickshell
  # speaks the greetd protocol natively (Quickshell.Services.Greetd) to
  # authenticate and launch `uwsm start hyprland-uwsm.desktop` — the same
  # session command tuigreet ran. dbus-run-session gives the greeter a
  # session bus (same launcher shape as the nixpkgs regreet module).
  # cage -s keeps VT switching available as the escape hatch if the greeter
  # ever fails to render (Ctrl+Alt+F2, then rollback).
