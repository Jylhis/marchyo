...
| `devenv shell` | Enter development shell with necessary tools (or `direnv allow`) |
...
1. Create the module file in the appropriate `modules/` subdirectory (`modules/nixos/` and `modules/home/` are auto-discovered; only `modules/darwin/` needs a manual import)
2. Define new options in a file under `modules/nixos/options/` (the directory is auto-discovered)
3. Add an evaluation test under `tests/eval/`
