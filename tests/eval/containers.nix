...
in
{
  ...
  eval-containers-no-docker-group-by-default = testNixOSCheck "containers-no-docker-group-by-default" (
    cfg: cfg.virtualisation.docker.enable && !(inDocker cfg)
  ) (withTestUser { marchyo.development.enable = true; });

  # Explicit opt-in: only when marchyo.development.containers.dockerGroup = true
  # is the user added to the `docker` group.
  eval-containers-docker-group-opt-in = testNixOSCheck "containers-docker-group-opt-in" (
    cfg: inDocker cfg
  ) (
    withTestUser {
  ...
  eval-containers-podman-rootless = testNixOSCheck "containers-podman-rootless" (
    cfg:
