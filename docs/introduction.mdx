...
The `nixosModules.default` output includes both NixOS system modules and Home Manager integration. If you only need Home Manager, use `homeManagerModules.default` instead.
...
Every custom option is defined under `modules/nixos/options/` — a directory of files split by namespace and auto-discovered — all under the `marchyo.*` namespace. Browse the [Configuration](/configuration/feature-flags) section for the full reference.
