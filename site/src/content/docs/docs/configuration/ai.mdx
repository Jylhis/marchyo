...
| `marchyo.ai.provider` | `"openrouter"` | AI service provider (enum; `"openrouter"` is the only value for now — reserved for future providers) |
...
| `marchyo.ai.routing.tasks.<bucket>` | pinned defaults | Per-bucket `{ model; fallbacks; }` — primary OpenRouter slug plus fallback chain |
| `marchyo.ai.routing.tools` | `{ aichat = "everydayCoding"; pi = "everydayCoding"; }` | Which task bucket each OpenRouter-backed client defaults to |
| `marchyo.ai.context.enable` | `false` | OpenViking local context layer |
| `marchyo.ai.context.workspacePath` | `".openviking/workspace"` | OpenViking workspace dir, relative to `$HOME` |
| `marchyo.ai.context.service.enable` | `false` | Run the OpenViking HTTP server as a per-user systemd service |
| `marchyo.ai.context.embeddingModel` | `openai/text-embedding-3-small` | OpenRouter embedding model slug used by OpenViking |
| `marchyo.ai.skills.enable` | `true` | Install Agent Skills to all clients |
| `marchyo.ai.mcp.enable` | `true` | Wire MCP tools into MCP-capable clients |
| `marchyo.ai.mcp.nixos.enable` | `true` | Enable the mcp-nixos server (run via uvx) |
...
`longContext`, `budget`, `local` (placeholder). Both aichat and pi default to the
`everydayCoding` bucket (`marchyo.ai.routing.tools`). The resolved policy is
exported to `~/.config/marchyo/ai-routing.json`, and each bucket is available as
an aichat role (`aichat -r frontier`).
