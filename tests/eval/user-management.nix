  ...
  eval-identity-darwin =
    testDarwinCheck "identity-darwin"
      (
        ...
        # nixos-only identities never leak into darwin group membership
        && !(lib.elem "linuxonly" cfg.users.groups.media.members)
  ...
  eval-identity-group-merge =
    testNixOSCheck "identity-group-merge"
      (
        ...
        # darwin-only identities never leak into NixOS group membership
        && !(lib.elem "maconly" cfg.users.groups.media.members)
        # literal names of accounts not managed by this module pass through
        && lib.elem "external" cfg.users.groups.media.members
